...
var generate_tooltip = function () {
  ...
  var init_tooltip = function () {
    ...
    var pos_left =
      getOffset(target).left + target.offsetWidth / 2 - tooltip.offsetWidth / 2;
    var pos_top = getOffset(target).top - tooltip.offsetHeight - 10;

    // Landing
    var landingClassNames = ["background", "active", "shown"];
    if (
      landingClassNames.some((className) =>
        getOffset(target).parent.classList.contains(className)
      )
    ) {
      pos_top =
        ...
        10 -
        (getOffset(target).parent.offsetHeight * 12) / 100;
    }
      ...
      render: {
        ...
        option: function (item, escape) {
          ...
          return (
            ...
            "</strong></p>" +
            (item.body
              ? '<p class="copied-field" style="display: none;">' +
                escape(item.body) +
                "</p>"
              : "") +
            "</div>"
      ...
      render: {
        item: function (item, escape) {
          return (
            "<div>" +
            "<p>" +
            (item.title ? escape(item.title) : "(Tanpa Grup)") +
            "</p>" +
            "</div>"
          );
        },
...
var guest_group_selection = function (
  el,
  data = [],
  sel = [],
  callback = () => {}
...
var hide_dropdown = function () {
  ...
  if (dropdownContent.length && $(dropdownContent).hasClass("show"))
    return $(dropdownContent).removeClass("show");
};
...
var init_tab = function (parent = "") {
  ...
  for (var i = 0; i < navs.length; i++) {
    if (
      parent != "" &&
      $(navs[i]).closest(parent).length &&
      $(navs[i]).hasClass("active")
    )
      $(navs[i]).trigger("click");
    if (parent == "" && $(navs[i]).hasClass("active"))
      $(navs[i]).trigger("click");
  }
};
...
var animateCSS = function (
  ...
  return new Promise((resolve, reject) => {
    ...
    $(element).addClass(
      animationInit + " " + animationName + " " + animationSpeed
    );
